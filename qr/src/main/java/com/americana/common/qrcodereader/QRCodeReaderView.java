...
import com.americana.common.R;
import com.americana.common.qrcodereader.camera.CameraSource;
import com.americana.common.qrcodereader.camera.CameraSourcePreview;
import com.americana.common.qrcodereader.camera.CustomQRCodeDetector;
import com.americana.common.utils.PermissionUtils;
...
public class QRCodeReaderView extends RelativeLayout implements View.OnClickListener, ExtractQRRedirectUrl.ExtractQRRedirectUrlCallbacks {
     ...
     * >>
     */
    private void updateOverlayHeight(Activity activity) {
        if (ivQrSquare != null) {
            ivQrSquare.post(() -> {
                if (callBacks == null)
                    return;
                ...
                if (widthRect != 0 && heightRect != 0 && isUpdateInPreviousSquarePosition()) {
                    ...
                    setAlignmentErrorMeasurment();
                    setLoaderViewWidthAndHeight();
     ...
     * >>
     */
    private void setBarcodeDetectorListner(String currentCountry, String deeplinkSchema) {
    ...
    private void startQRAlignmentHandler() {
        ...
        if (qrAlighHandler == null) {
            ...
            qrAlignRunnable = () -> showAlingmentError(true);
        }
     ...
     */
    public void updateSquarePosition(Activity activity) {
        new Handler().postDelayed(() -> {
                ...
                if (toastOne != null) {
                    try {
                        if (toastOne.getView().isShown())     // true if visible
                        {
                            ...
                            inflateToastLayout(message, context);
                        }
    ...
    private void scanQrcodes(InputImage image) {
        ...
        Task<List<com.google.mlkit.vision.barcode.Barcode>> result = scanner.process(image)
                .addOnSuccessListener((OnSuccessListener<List<com.google.mlkit.vision.barcode.Barcode>>) barcodes -> {
                    // Task completed successfully
                    com.google.mlkit.vision.barcode.Barcode barcode = getKfcBarCodeFromGalleryImage(barcodes);
                    if (barcode == null) {
                        if (barcodes.size() > 1)
                            showCustomToast(getContext(), getMultipleQRError());
                        else
                        ...
                        extractDeepLink(url, true, currentCountry, deepLinkSchema);
                    }
                })
     ...
     */
    private void openQRErrorAlertDialog(int errorType) {
        new Handler(Looper.getMainLooper()).post(() -> {
            ...
            String title = "";
            String msg = "";
            String negativeBtnText = getUnableToReadQRAlertNegativeBtnText();
            String postiveBtnText = getUnableToReadQRAlertPositiveBtnText();

            switch (errorType) {
                case QR_ERROR_NOTHING_TO_READ:
                case QR_ERROR_NO_DEEP_LINK:
                case QR_ERROR_DEEP_LINK_EXPIRE:
                    msg = getUnableToReadQRAlertDescription();
                    title = getUnableToReadQRAlertTitle();
                    break;
            }
     ...
     */
    public void extractDeepLink(String deepLinkUrl, boolean isFromGallery, String currentCountry, String deepLinkScheme) {
        new Handler(Looper.getMainLooper()).post(() -> {
            if (callBacks != null && extractQRInstance != null) {
                ...
                extractQRInstance.extractDeepLink(deepLinkUrl, getContext(), isFromGallery, currentCountry, deepLinkScheme);
            } else {
                isProcessingQR = false;
            }
        });
